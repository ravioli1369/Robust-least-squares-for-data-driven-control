...
%% 2. Define the "Unknown" System (Laplacian system)
Ts = 0.01; % Sample time
A_c = [[1, 1, 0]; [1, 1, 1]; [0, 1, 1]];
B_c = eye(3)/Ts;
C_c = [1, 0, 0];
D_c = [0, 0, 0];
...
n = size(A_d, 1); % System order (n=3)
m = size(B_d, 2); % Input dim (m=3)
...
T_data = 150; % Total length of data collection
...
%% 7. Run Simulation
T_sim = 100; % Simulation length
fprintf('Running simulation for %d steps...\n', T_sim);

% Reference trajectory
r_sim = zeros(p, T_sim + N_f);
...
x_hist(:, 1) = [-1.5; -0.5796;-2.7460]; % True initial state
u_past_buffer = zeros(m, T_ini);
y_past_buffer = zeros(p, T_ini);

x_current = [-1.5; -0.5796;-2.7460]; % same IC for comparison
% --- Simulation Loop ---
